        ...
        <ScrollView contentContainerStyle={{paddingBottom: 20}} className={'flex-1 bg-slate-900'}>
            <View className={"w-full"}>
                ...
                {isLoading ? <Loader/> : <View>
                        <Image source={{uri: image500(movie.poster_path)}} style={{width,height:height*0.9}}/>
                ...
                <View className={"flex-row justify-between mx-4 space-x-2"}>
                    {movie?.genres?.map((genres,idx) => (
                        <Text key={idx} className={"text-white font-semibold text-base text-center"}>
                            {idx !== movie.genres.length ? "*" : null} {''} {genres?.name}
                        </Text>
                    ))}
                </View>
            ...
            {movie?.id && cast.length > 0 && <UpcomingMovie upcoming={similarMovie} title={"Similar movies"}/>}
        </ScrollView>
    )
}