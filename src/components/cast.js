...
export default function Cast({cast}){
    ...
    return(
        <View className={"my-6"}>
            <Text className={"text-white text-lg mx-4 mb-5 mt-4"}>Actors</Text>
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{paddingHorizontal: 15}}>
                {cast && cast.map((person, idx)=>(
                    <TouchableOpacity onPress={()=>navigation.navigate("Person", person.id)} key={idx} className={"mr-4 items-center"}>
                        ...
                        {/*text-xs kichikroq qilib beradi itemdi*/}
